#!/usr/bin/env python
""" MURED training script

Adapted from train_prism_eb6_ddp.py (KAMATALAB RIADD baseline) for the MURED dataset.
Same model/augmentation/loss/DDP recipe; single train/val split (MURED ships only
train/test, so a fixed-seed 90/10 holdout of train serves as val) instead of RFMiD's
5-fold CV.
"""
import argparse
import time
...
from timm.data import PrismDataSet as MuredDataSet
...
parser = argparse.ArgumentParser(description='PyTorch MURED Training')

# Dataset / Model parameters
parser.add_argument('--train-data', default='/storage2/cousin/datasets/MURED/images/images', type=str,
                    metavar='DIR', help='path to training images')
parser.add_argument('--val-data', default='/storage2/cousin/datasets/MURED/images/images', type=str,
                    metavar='DIR', help='path to validation images')
parser.add_argument('--train-csv', default='/storage2/cousin/datasets/MURED/train_labels.csv', type=str,
                    metavar='PATH', help='path to training labels csv (filename + label columns)')
parser.add_argument('--val-csv', default='/storage2/cousin/datasets/MURED/val_labels.csv', type=str,
...
parser.add_argument('--output', default='ckpt/tf_efficientnet_b6_ns-MURED-SGD1E-1', type=str, metavar='PATH',
...
def main():
    ...
    dataset_train = MuredDataSet(image_ids=train_data, baseImgPath=args.train_data)
    dataset_eval = MuredDataSet(image_ids=val_data, baseImgPath=args.val_data)
