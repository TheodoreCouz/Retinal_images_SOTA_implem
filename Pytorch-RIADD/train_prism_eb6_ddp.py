...
Adapted from train_riadd_eb6_ddp.py (KAMATALAB RIADD baseline) for the PRISM v1 dataset.
Same model/augmentation/loss/DDP recipe; single train/val split (PRISM provides its own
train/val/test partition, so no KFold here) instead of RFMiD's 5-fold CV.
"""
import argparse
import time
import yaml
...
from timm.data import PrismDataSet
...
parser = argparse.ArgumentParser(description='PyTorch PRISM Training')

# Dataset / Model parameters
parser.add_argument('--train-data', default='/home/cousin/research/Fiber_dino/PRISM v1/train', type=str,
                    metavar='DIR', help='path to training images')
parser.add_argument('--val-data', default='/home/cousin/research/Fiber_dino/PRISM v1/val', type=str,
                    metavar='DIR', help='path to validation images')
parser.add_argument('--train-csv', default='/home/cousin/research/Fiber_dino/PRISM v1/train_labels.csv', type=str,
                    metavar='PATH', help='path to training labels csv (filename + label columns)')
parser.add_argument('--val-csv', default='/home/cousin/research/Fiber_dino/PRISM v1/val_labels.csv', type=str,
                    metavar='PATH', help='path to validation labels csv')
parser.add_argument('--model', default='tf_efficientnet_b6_ns', type=str, metavar='MODEL',
                    help='Name of model to train')
parser.add_argument('--pretrained', action='store_true', default=False,
                    help='Start with pretrained version of specified network (if avail)')
parser.add_argument('--initial-checkpoint', default='./preweights/tf_efficientnet_b6_ns-51548356.pth',
...
parser.add_argument('--num-classes', type=int, default=31, metavar='N',
                    help='number of label classes')
parser.add_argument('--gp', default=None, type=str, metavar='POOL',
                    help='Global pool type, one of (fast, avg, max, avgmax, avgmaxc). Model default if None.')
parser.add_argument('--img-size', type=int, default=960, metavar='N',
...
parser.add_argument('--output', default='ckpt/tf_efficientnet_b6_ns-PRISM-SGD1E-1', type=str, metavar='PATH',
...
def gather_predict_label(eval_metrics, args):
    if args.distributed:
...
def main():
    ...
    dataset_train = PrismDataSet(image_ids=train_data, baseImgPath=args.train_data)
    dataset_eval = PrismDataSet(image_ids=val_data, baseImgPath=args.val_data)
