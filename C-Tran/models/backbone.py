...
class Backbone(nn.Module):
    """CNN feature extractor for C-Tran.

    Supports the original ResNet101 as well as DenseNet161, which the
    MuReD paper (Rodriguez et al. 2023) found to be the best-performing
    feature extractor. Exposes `out_channels` so the transformer hidden
    size can adapt to the backbone (2048 for ResNet101, 2208 for DenseNet161).
    Output is a spatial feature map (B, out_channels, H, W).
    """
    def __init__(self, backbone='densenet161'):
        super(Backbone, self).__init__()
        self.backbone_name = backbone

        if backbone == 'resnet101':
            self.base_network = models.resnet101(
    ...
    def forward(self, images):
        if self.backbone_name == 'resnet101':
            ...
            x = F.relu(x, inplace=True)  # torchvision applies ReLU after features
            return x
