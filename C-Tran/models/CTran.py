...
class CTranModel(nn.Module):
    def __init__(self,num_labels,use_lmt,pos_emb=False,layers=3,heads=4,dropout=0.1,int_loss=0,no_x_features=False,backbone='densenet161'):
        super(CTranModel, self).__init__()
        self.use_lmt = use_lmt

        self.no_x_features = no_x_features # (for no image features)

        # CNN backbone (ResNet101 or DenseNet161)
        self.backbone = Backbone(backbone)
        hidden = self.backbone.out_channels # match the backbone output feature size
